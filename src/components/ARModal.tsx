...
const getCameraErrorMessage = (error: unknown): string => {
    if (error instanceof DOMException) {
        if (error.name === 'NotAllowedError' || error.name === 'PermissionDeniedError') {
            return 'Activa el permiso de camara para mostrar el fondo AR.';
        }

        if (error.name === 'NotFoundError' || error.name === 'OverconstrainedError') {
            return 'No se encontro una camara frontal disponible en este dispositivo.';
        }
    }

    return 'No fue posible abrir la camara en este dispositivo Android.';
};
...
const ARModal: FC<ARModalProps> = ({ tipo, contenido, fondo, onClose }) => {
    ...
    useEffect(() => {
        ...
        // Cámara trasera (solo acierto)
        const startCameraFeed = async () => {
            if (tipo !== 'acierto' || !videoFeedRef.current) return;
        ...
        <div className="ar-modal-overlay" onClick={onClose}>
            ...
            >
                ...
                {tipo === 'acierto' && cameraError && (
                    <div className="ar-camera-status">{cameraError}</div>
                )}
